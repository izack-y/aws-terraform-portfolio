...
* **コード整理**: 変更可能性がある値(regionなど)を変数定義し、それを **`variables.tf`** ファイルにまとめた。それによって、main.tf(リソース定義)の可読性の向上。
...
### 3. RDSインスタンスのデプロイ中にエラー
**エラー**DB サブネットグループがアベイラビリティーゾーン (AZ) のカバレッジ要件を満たしていません。という旨のエラーが発生。
* **原因**：AWS RDS（リレーショナルデータベースサービス）は、高い可用性（サービスが停止しないこと）を確保するため、データベースインスタンスを配置するDBサブネットグループに対して、最低 2 つのアベイラビリティゾーン（AZ）でのプライベートサブネットが必要。
